	...
	protected void prepareRateFileForFrontend(RateFile rateFile) {
		...
		for (Zone z : rateFile.getZones()) {
			if (z.getZoneType() == ZoneType.ALPHANUMERIC_LIST) {
				z.setAlphaNumericPostalCodesAsString(zoneUtil
						.convertAlphaNumericPostalCodeListToString(z
								.getAlphaNumericalPostalCodes()));
			} else {
				z.setNumericalPostalCodesAsString(zoneUtil
						.convertNumericalPostalCodeListToString(z

...
public class ExcelToModelMapper {
   ...
   * 
   * @param rf the ratefile to map to.
   * @param data the data used to map from.
   */
  ...
  protected List<Condition> createConditions(ExcelUploadUtilData data, RateFile rf) {
  ...
  protected List<RateLine> createRateLines(ExcelUploadUtilData data, RateFile rf) {
  ...
  private void fillInPostalCodes(RateFile rf, ExcelUploadUtilData data, Integer key, Zone zone) {
  ...
  private String constructPostalCodeString(List<String> postalCodes) {
    if (postalCodes != null) {
      ...
      postalCodeAsString = postalCodeAsString.substring(0, postalCodeAsString.length() - 1);

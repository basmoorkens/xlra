...
public class RaiseRateServiceTest extends UnitilsJUnit4 {
  ...
  public void init() {
    ...
    raiseRateFileServiceImpl.setLogRecordFactoryService(logRecordFactory);
  ...
  public void testraiseRateFiles() {
    raiseRateFileServiceImpl.raiseRateFiles(10d, Arrays.asList(rateFile), RateOperation.RAISE);
  ...
  public void testCancelLastRaiseNoRaiseFound() {
    EasyMock.expect(logService.getLastRaiseRates()).andReturn(null);
    EasyMockUnitils.replay();
  ...
  public void testRaiseRatesWithPercentage() {
    EasyMock.expect(rateFileDaoMock.getFullRateFile(rateFile.getId())).andReturn(rateFile);
    EasyMock.expect(
        logRecordFactory.createRaiseRatesRecord(RateOperation.RAISE, 10d, Arrays.asList(rateFile)))
        .andReturn(new RaiseRatesRecord());
    ...
    EasyMockUnitils.replay();
    raiseRateFileServiceImpl.raiseRateFileRateLinesWithPercentage(Arrays.asList(rateFile), 10d);
    Assert.assertEquals(calcUtil.roundBigDecimal(new BigDecimal(110.00d)), rateFile.getRateLines()
        .get(0).getValue());
    Assert.assertEquals(calcUtil.roundBigDecimal(new BigDecimal(165.00d)), rateFile.getRateLines()
        .get(1).getValue());
  }
  ...
  public void testApplyRateOperation() {
    ...
    EasyMockUnitils.replay();
    raiseRateFileServiceImpl.applyRateOperation(Arrays.asList(rateFile), 10d, RateOperation.RAISE);
    Assert.assertEquals(calcUtil.roundBigDecimal(new BigDecimal(110.00d)), rateFile.getRateLines()
        .get(0).getValue());
    Assert.assertEquals(calcUtil.roundBigDecimal(new BigDecimal(165.00d)), rateFile.getRateLines()
        .get(1).getValue());
  ...
  public void testApplyRateOperationNegative() {
    ...
    EasyMock.expect(rateFileDaoMock.getFullRateFile(rateFile.getId())).andReturn(rateFile);
    EasyMock.expect(
        logRecordFactory.createRaiseRatesRecord(RateOperation.SUBTRACT, 10d,
            Arrays.asList(rateFile))).andReturn(new RaiseRatesRecord());
    logService.createLogRecord((LogRecord) EasyMock.anyObject());
    EasyMock.expect(rateFileDaoMock.updateRateFile(rateFile)).andReturn(rateFile);

    EasyMockUnitils.replay();
    raiseRateFileServiceImpl.applyRateOperation(Arrays.asList(rateFile), 10d,
        RateOperation.SUBTRACT);
    Assert.assertEquals(calcUtil.roundBigDecimal(new BigDecimal(90.00d)), rateFile.getRateLines()
        .get(0).getValue());
    Assert.assertEquals(calcUtil.roundBigDecimal(new BigDecimal(135.00d)), rateFile.getRateLines()
        .get(1).getValue());
  }
}

...
public class RateFileServiceTest extends UnitilsJUnit4 {
	...
	public void testGetRateFilesForFilter() {
		...
		EasyMock.expect(rfDao.getRateFilesForFilter(filter)).andReturn(
				new ArrayList<RateFile>());
		EasyMockUnitils.replay();
	...
	@Test
	public void testgetRateFileQueryFullCustomerRateFileNotFound()
	...
	public void testgetCopyOfRateFileForFilter() throws RateFileException {
		...
		EasyMock.expect(rfDao.getFullRateFileForFilter(filter)).andThrow(
				new NoResultException());
		EasyMock.expect(
				rfDao.getFullRateFileForFilter(EasyMock
						.isA(RateFileSearchFilter.class))).andReturn(rf1);
		EasyMockUnitils.replay();
		RateFile rs = rateFileService
				.generateCustomerRateFileForFilterAndCustomer(filter);
		Assert.assertNotNull(rs);
		Assert.assertEquals("testje - be - Kilo - null - null", rs.getName());
	}

	@Test(expected = RateFileException.class)
	public void testgetCopyOfRateFileForFilterAlreadyExistsForCustomer()
			throws RateFileException {
		RateFile rf1 = new RateFile();
		Customer customer = new Customer();
		customer.setName("testje");
	...
	public void testGetRateFilesbyIdWithEmptyIdList() {
		...
		EasyMock.expect(rfDao.getRateFilesById(ids)).andReturn(
				new ArrayList<RateFile>());
		EasyMockUnitils.replay();
